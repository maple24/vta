...
def with_queue_management(queue_attr: str, event_attr: str):
...
class PuttyHelper(SerialInterface):
    ...
    def is_connected(self) -> bool:
        ...
        with self._lock:
            return (self._state == ConnectionState.CONNECTED and 
                   self._connection and self._connection.is_open)
        ...
        try:
            while self._reader_event.is_set():
                ...
                try:
                    ...
                    if line:
                        logger.debug("[PuttyRx] - {message}", message=line)
                        timestamp = time.time()
            ...
            logger.debug("Serial reader thread terminated")
    ...
    @with_queue_management('_wait_queue', '_wait_event')
    ...
    def _wait_for_pattern(self, pattern: str, command: str = "", timeout: float = 10.0, auto_login: bool = True) -> Tuple[bool, Optional[Tuple]]:
    ...
    def wait_for_trace(self, pattern: str, command: str = "", timeout: float = 10.0, auto_login: bool = True) -> Tuple[bool, Optional[List]]:
        """
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                success, _ = self._wait_for_pattern(
                    "(Password:.*)|(Logging in with home .*)",
                    self._config.username, 5, False
                )
                if not success:
                    continue
