...
def get_stored() -> list:
    ...
    stored = [dict(zip(['id', 'summary', 'created', 'status'], row)) for row in cursor.fetchall()]
    return stored
...
def synchronize_data(data_from_source: list):
    ...
    for existing_entry in existing_data:
        ...
        source_entry = next((item for item in data_from_source if item["id"] == id_to_check), None)
        if not source_entry:
