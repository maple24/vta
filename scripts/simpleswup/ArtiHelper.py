...
class Aritifacoty_Download:
    ...
    def __init__(self, server, repo_path, auth=('ets1szh', 'estbangbangde5'), dstfolder='Download') -> None:
        self.server = server
        self.auth = auth
        ...
        logging.basicConfig(level=logging.DEBUG, filename=os.path.join(self.dstfolder, 'multithread_dl.log'),
    ...
    def get_latest_version(self, branch):
        ...
        latest_version = branch.split('/')[-1] + "_" + str(latest_year) + "." + str(latest_week) + "." + str(
            latest_pack)
                    ...
                    with self.__file_lock:
                        ...
                        if self.__threads_status[thread_name]['status'] == 0:
                            if page["start_pos"] < page["end_pos"]:
                                print("|- {}  Downloaded: {}MB / {:.2f}MB".format(thread_name, chunk_num,
                                                                                  self.__threads_status[thread_name][
                                                                                      'page_size'] / 1024 / 1024))
                            else:
...
def artimonitor(server, repo, pattern, auth=('ets1szh', 'estbangbangde5')):
